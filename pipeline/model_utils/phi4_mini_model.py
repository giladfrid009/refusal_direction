...
PHI4_CHAT_TEMPLATE = """<|user|>{instruction}<|end|><|assistant|>"""

PHI4_CHAT_TEMPLATE_WITH_SYSTEM = """<|system|>{system_prompt}<|end|><|user|>{instruction}<|end|><|assistant|>"""

PHI4_REFUSAL_TOKS = [15390, 33680]  # [I'm, Sorry]
...
def format_instruction_llama32_chat(instruction: str, output: str = None, system: str = None, include_trailing_whitespace: bool = True):
    if system is not None:
        formatted_instruction = PHI4_CHAT_TEMPLATE_WITH_SYSTEM.format(instruction=instruction, system_prompt=system)
    else:
        formatted_instruction = PHI4_CHAT_TEMPLATE.format(instruction=instruction)
...
class Phi4MiniModel(ModelBase):
    ...
    def _get_eoi_toks(self):
        return self.tokenizer.encode(PHI4_CHAT_TEMPLATE.split("{instruction}")[-1], add_special_tokens=False)

    def _get_refusal_toks(self):
        return PHI4_REFUSAL_TOKS
