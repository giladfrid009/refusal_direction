...
R2D2_CHAT_TEMPLATE = """<|user|>
{instruction}</s> 
<|assistant|>
"""

R2D2_CHAT_TEMPLATE_WITH_SYSTEM = """<|system|>
{system_prompt}</s> 
<|user|>
{instruction}</s> 
<|assistant|>
...
R2D2_REFUSAL_TOKS = [23166, 28737]  # [Sorry, I]


def format_instruction_r2d2_chat(instruction: str, output: str = None, system: str = None, include_trailing_whitespace: bool = True):
    if system is not None:
        formatted_instruction = R2D2_CHAT_TEMPLATE_WITH_SYSTEM.format(instruction=instruction, system_prompt=system)
    else:
        formatted_instruction = R2D2_CHAT_TEMPLATE.format(instruction=instruction)

    if not include_trailing_whitespace:  # TODO: remove this everywhere
...
def tokenize_instructions_llama3_chat(
    ...
    if outputs is not None:
        ...
        prompts = [
            format_instruction_r2d2_chat(instruction=instruction, system=system, include_trailing_whitespace=include_trailing_whitespace)
...
class ZephyrR2D2Model(ModelBase):
    ...
    def _get_eoi_toks(self):
        return self.tokenizer.encode(R2D2_CHAT_TEMPLATE.split("{instruction}")[-1], add_special_tokens=False)

    def _get_refusal_toks(self):
        return R2D2_REFUSAL_TOKS
